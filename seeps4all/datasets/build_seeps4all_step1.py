...
# location identifier
def location_identifier(icnt, lats, lons):
    return int(lats[icnt] * 10000) * 1000 + lons[icnt]


unit_convert = {"RR": 10}  # unit in ecad is 0.1 mm, so we have to devide by 10


def main(path_input: str, path_output: str, element: str = "RR", year_clim_i: int = 1991, year_clim_f: int = 2020):
    if element not in unit_convert:
        raise ValueError(f"{element=} is not implemented, known elements={list(unit_convert)}")
    ...
    # Read ECAD file
    info = pd.read_csv(
        f"{path_input}/stations.txt", sep=",", header=25, names=("STAID", "STANAME", "CN", "LAT", "LON", "HGHT")
    )
    ...
    for icnt, stnid in enumerate(stnid_list):
        ...
        # read station file
        filein = os.path.join(path_input, f"{element}_STAID0{stnid:05}.txt")
        data = pd.read_csv(filein, sep=",", header=22, names=("STAID", "SOUID", "DATE", element, f"Q_{element}"))

        # get preicpitation values and quality control values
        val = np.array([float(v) / unit_convert[element] for v in data[element]])
        qc = np.array([int(v) for v in data[f"Q_{element}"]])
        ...
        if location_identifier(icnt, lats, lons) in latlons:
            continue
        else:
            latlons.append(location_identifier(icnt, lats, lons))
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser("convert ecad to seeps4ll for obs_seeps and obs_clim")
    parser.add_argument("--path_input", type=str, help="path to read ecad files from")
    parser.add_argument("--path_output", type=str, help="path to write transformed ecad files to")
    parser.add_argument("--element", type=str, help="element type", default="RR")
    args = parser.parse_args()
    main(args.path_input, args.path_output, args.element)
