...
import pandas as pd
import xarray as xr
from build_seeps4all_step1 import location_identifier, unit_convert
...
def main(
    ...
    for icnt, stnid in enumerate(stnid_list):
        ...
        data = pd.read_csv(filein, sep=",", header=22, names=("STNID", "SOUID", "DATE", element, f"Q_{element}"))
        ...
        clim_eq = pd.read_csv(
            f"{path_output}/local_climate_{clim_years}/equitable_categories_st{stnid}_{clim_years}.csv"
        )
        if len(clim_eq) < 12:
        ...
        for ida, dd in enumerate(tim[dok]):
            ...
            if (
                (p1 < 0.1)
                | (p1 >= 0.85)
                | (clim_eq["count_all"].iloc[montho - 1] < 300)
                | (clim_eq["count_t1"].iloc[montho - 1] < 50)
            ):
                p1, p3 = np.nan, np.nan
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser("convert ecad to seeps4ll")
